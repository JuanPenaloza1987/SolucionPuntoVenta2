...
namespace SRATAPV.Ventas.Negocio
{
    class clsCorte
    {
        ...
        public DataSet leerVentasDetalleDelCorte(string idsVen)//No se usa
        {
            ...
            Objeto.sentenciaSQL = "SELECT ven4_metodopago AS metodopago ,SUM(ven4_importe) AS importe FROM procvent4 WHERE ven4_keyven IN ("+idsVen+") GROUP BY ven4_metodopago";
            consulta = Objeto.ejecutaConsulta();
            if (!Objeto.hayError)
            {
                return consulta;
